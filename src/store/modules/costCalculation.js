    ...
    NEXT_STEP({ commit, dispatch, state }, step) {
      ...
      switch (step) {
        ...
        case 'FromWhichCountry':
          commit('SET_COST_CALCULATION_APPL', {
            id: 'from_country',
            value: this.getters.INPUT
              ? this.getters.INPUT
              : this.getters.COST_CALCULATION_APPL.from_country,
          });
        ...
        case 'cities':
          commit('SET_COST_CALCULATION_APPL', {
            ...
            value: this.getters.INPUT
              ? this.getters.INPUT
              : this.getters.COST_CALCULATION_APPL.from_city,
          });
          commit('SET_INPUT', currentRoute);
        ...
        case 'naming':
          commit('SET_COST_CALCULATION_APPL', {
            ...
            value: this.getters.INPUT
              ? this.getters.INPUT
              : this.getters.COST_CALCULATION_APPL.name,
          });
          commit('SET_INPUT', currentRoute);
        ...
        case 'ApplicationDescription':
          commit('SET_COST_CALCULATION_APPL', {
            ...
            value: this.getters.INPUT
              ? this.getters.INPUT
              : this.getters.COST_CALCULATION_APPL.description,
          });
          commit('SET_INPUT', currentRoute);
  ...
  mutations: {
    ...
    CLEAR_COST_CALCULATION_APPL(state) {
      for (let el in state.cost_calculation_appl) {
        switch (el) {
          case 'to_country':
            state.cost_calculation_appl[el] = 'Россия';
            break;
          case 'amount':
            state.cost_calculation_appl[el] = 0;
            break;
          case 'additional_services':
            state.cost_calculation_appl[el] = [];
            break;
          default:
            state.cost_calculation_appl[el] = '';
        }
