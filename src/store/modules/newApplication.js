...
export default {
  state: {
    ...
    comment_for_service: '',
    delivery_type:
      'Оптимальный (золотая середина между скоростью и стоимостью)',
    addressFrom: {
      from_city: '',
      from_address: '',
    },
  ...
  actions: {
    ...
    async CREATE_CUSTOMER_ORDER({ state, commit }) {
      ...
      const order_packing_list =
        this.getters.DOCS.order_packing_list.binaryFile;
  ...
  mutations: {
    SET_INPUT(state, route) {
      if (
        route.path === '/' ||
        state.selectedId === 'country' ||
        route.path === '/start-application' ||
        route.path === '/cost-calculation'
      ) {
        state.country = this.getters.INPUT
          ? this.getters.INPUT
          : this.getters.COUNTRY;
        this.commit('SET_VALUE_FOR_SELECTED_ORDER', {
          key: 'from_country',
          value: state.country,
        });
      } else if (
        route.query.step === 'cities' ||
        state.selectedId === 'cities'
      ) {
        state.city = this.getters.INPUT
          ? this.getters.INPUT
          : this.getters.CITY;
        this.commit('SET_VALUE_FOR_SELECTED_ORDER', {
          key: 'to_city',
        ...
        route.query.step === 'naming' ||
        state.selectedId === 'naming'
      ) {
        state.application_name = this.getters.INPUT
          ? this.getters.INPUT
          : this.getters.APPLICATION_NAME;
        this.commit('SET_VALUE_FOR_SELECTED_ORDER', {
          key: 'name',
          value: state.application_name,
        });
      } else if (
        route.query.step === 'ApplicationDescription' ||
        state.selectedId === 'description'
      ) {
        state.description = this.getters.INPUT
          ? this.getters.INPUT
          : this.getters.APPLICATION_DESCRIPTION;
        this.commit('SET_VALUE_FOR_SELECTED_ORDER', {
          key: 'description',
    ...
    SET_SERVICE(state, { checked, service }) {
      if (checked) {
        ...
        this.commit('SET_COST_CALCULATION_APPL', {
          id: 'additional_services',
          value: state.additional_services,
        });
      } else {
        state.additional_services = state.additional_services.filter(
          (el) => el !== service
        );
        this.commit('SET_COST_CALCULATION_APPL', {
          id: 'additional_services',
    ...
    SET_ADDITION_DATA(state, data) {
      debugger;
      if (
        data.nameOfInput === 'from_city' ||
        data.nameOfInput === 'from_address'
      ) {
        state.addressFrom[data.nameOfInput] = data.inputValue;
      } else if (
        data.nameOfInput === 'gross' ||
        data.nameOfInput === 'cargo_volume' ||
        data.nameOfInput === 'date_of_cargo_readiness' ||
        data.nameOfInput === 'amount'
      ) {
    ...
    SET_COMMENT_FOR_SERVICE(state, comment) {
      ...
      this.commit('SET_COST_CALCULATION_APPL', {
        id: 'comment_for_service',
        value: state.comment_for_service,
      });
      this.commit('SET_VALUE_FOR_SELECTED_ORDER', {
        key: 'comment_for_service',
        value: comment,
      });
    },
