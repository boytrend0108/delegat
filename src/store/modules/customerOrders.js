  ...
  mutations: {
    ...
    SET_EMUM_SERVICES_FOR_SELECTED_ORDER(state) {
      if (state.selected_order.additional_services) {
        state.selected_order.additional_services = enumServices2(
          state.selected_order.additional_services
        );
      }
      if (state.selected_order.delivery_type) {
        state.selected_order.delivery_type = enumDeliveryType2(
          state.selected_order.delivery_type
        );
      }
    },
    ...
    SET_SERVICES_FOR_SELECTED_ORDER(state, { checked, service }) {
      if (checked) {
        ...
        state.selected_order.additional_services =
          state.selected_order.additional_services.filter(
            (el) => el !== service
          );
      }
    },
