...
export default {
  ...
  actions: {
    async GET_COMPANIES_FROM_DADATA({ commit }, inputInn) {
      // for dropdown
      await axios({
        method: 'post',
    ...
    async GET_BANKS_FROM_DADATA({ commit }, inputInn) {
      // for dropdown
      await axios({
        method: 'post',
  ...
  mutations: {
    ...
    SET_ADD_OR_CHANGE_FIRM(state, action) {
      // action === addNewCompany or changeFirm
      state.addOrChangeFirm = action;
    },

    SET_ADD_OR_CHANGE_BANK(state, action) {
      // action === addNewCompany or changeFirm
    ...
    SET_COMPANY_DATA(state, data) {
      // добавляем телефон и почту компании и факт.адресс
      state.firms[0][data.key] = data.value;
    },

    SET_NEW_COMPANY_FORM(state, inn) {
      //ucпользуется при заполнении анкеты посльзователем
      const company = state.companiesList2.filter(
        (firm) => inn === firm.data.inn
      )[0];
      const companyFormData = {
        ...
        director_name: company.data.management
    ...
    SET_BANK_FORM(state, data) {
      // cобираем форму для запроса
    ...
    SET_COMPANIES_LIST(state, company) {
      if (company) {
        state.companiesList.push(
          `${company.data.inn}` + ` | ` + `${company.value}`
        );
