...
export default {
  ...
  actions: {
    ...
    async GET_PROVIDER_OFFERS({ commit }, order_id) {
    ...
    async GET_MY_OFFERS(_, { status, page_size, page }) {
      return new Promise((resolve) => {
        axios({
          ...
          .catch((err) => {
            console.log('Ошибка загрузки откликов исполнителя', err);
          });
      });
    },
  ...
  mutations: {
      ...
      switch (action) {
        case 'add':
          ...
          localStorage.setItem(
            'unwatched_offers',
            JSON.stringify(state.new_offer_in_progress)
          );
          break;
        case 'delete':
          state.new_offer_in_progress = state.new_offer_in_progress.filter(
            (el) => el !== +data
          );
          localStorage.setItem(
            'unwatched_offers',
          ...
          break;
        case 'set':
          state.new_offer_in_progress = data;
      }
    },
    ...
    SET_LEAVE_SELECTED_OFFER(state, offer_id) {
      state.provider_offers = state.provider_offers.filter(
        (el) => el.id == offer_id
      );
    },
    ...
    SET_IS_PROVIDER_SELECTED(state, data) {
      ...
      if (router.currentRoute.value.name === 'provider-application') {
        ...
        router.push(
          `/provider/application/${id}?status=IN_PROGRESS&feedback=true&offer_id=${offer_id}`
        );
      }
    },
