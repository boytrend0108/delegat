...
export default {
  state: {
    ...
    avatar_by_id: '@/assets/images/icons/user.svg',
    user_by_id: '',
  },
  ...
  actions: {
    async GET_USER_BY_ID({ commit }, user_id) {
      return new Promise((resolve) => {
        axios({
          ...
          .catch((err) =>
            console.log('Oшибка получения пользователя по id', err)
          );
      });
    },
    ...
    async UPLOAD_AVATAR_TO_SERVER() {
      ...
      await axios({
        ...
        console.log('Ошибка при загрузке аватара на сервер', err)
      );
    },

    async GET_AVATAR_FROM_SERVER({ commit }) {
      // для получения своего аватара
      await axios({
        method: 'get',
        url: '/me/picture',
      })
    ...
    async GET_AVATAR_BY_ID({ commit }, user_id) {
      // для получения aватара контр-агента
      return new Promise((resolve) => {
        axios({
          method: 'get',
          ...
          .catch((err) =>
            console.log('Ошибка загрузки аватара контр-агента', err)
          );
      });
    },
