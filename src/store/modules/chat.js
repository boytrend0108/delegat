...
export default {
  state: {
    ...
    unreadMessages: [],
    chat_page: 0,
    guest_avatar: '@/assets/images/icons/user.svg',
    bigImgChat: '',
    newMsg: '',
    techMsg:
      'Вы не можете написать в этот чат, так как Заказчик уже выбрал Исполнителя. В случае отказа от Исполнителя у Вас вновь появится возможность общаться в этом чате',
    isChatClosed: false,
    rating: {},
    isMsgRead: null,
    clipboard: '',
  ...
  actions: {
    ...
    async READ_MSG(_, data) {
      await axios({
        ...
        console.log(err, 'Ошибка отправки отчета о прочитанном сообщении');
      });
    },
  ...
  mutations: {
    ...
    SET_UNREADED_MESSAGES(state, { data, action }) {
      ...
      const indexForDeleting = state.unreadMessages.findIndex(
        (el) => el.chat_id === data[0]?.chat_id
      );
      switch (action) {
        case 'add':
    ...
    SET_GUEST_AVATAR(state, avatar_url) {
      if (avatar_url === null)
        state.guest_avatar = require('@/assets/images/icons/user.svg');
