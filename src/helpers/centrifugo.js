...
export function connectToCentrifuge() {
  const centrifuge = new Centrifuge(
    'wss://socket.delegat.online/connection/websocket',
  ...
  centrifuge.on('publication', function (ctx) {
    ...
    if (payload.new_message.chat_id === store.getters.CHAT_ID)
      store.commit('SET_NEW_MSG', payload);
  });

  centrifuge.connect();

  const sub = centrifuge.newSubscription(
    `personal:#${store.getters.CURRENT_USER.uid}`
  );
  ...
  sub.on('publication', function (ctx) {
    ...
    if (ctx.data.read_message) {
      if (
        router.currentRoute.value.name === 'customer-chat' ||
        router.currentRoute.value.name === 'provider-chat'
      ) {
    ...
    if (
      store.getters.SITE_ROLE === 'customer' &&
    ...
    if (
      ctx.data.new_message.is_service &&
      ctx.data.new_message.user_id === store.getters.CURRENT_USER.uid
    ) {
    ...
    if (
      ctx.data.new_message.chat_id === store.getters.CHAT_ID &&
      !ctx.data.new_message.is_service
    ) {
      store.commit('SET_NEW_MSG', ctx.data.new_message);
    }
